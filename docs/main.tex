...
Tracking the logit lens projection for hallucinated target tokens (e.g., `"mouse"`) reveals that token probability remains near zero throughout layers 0--23, followed by an acute spike at \textbf{Layer 25} (peaking at $p = 0.028$ before final softmax calibration). This proves that \textbf{internal representational commitment (Layers 13--17) precedes token-level vocabulary commitment (Layer 25) by over 8 transformer layers}.
...
\midrule
\textbf{Positive Controls (Lobotomy Check)} & 100.0\% & \textbf{85.7\% (12/14)} & High retention of genuine object recognition \\
\textbf{Absence Traps (Counter-Existence)} & 0.0\% (Hallucinates) & \textbf{Truthful Negation} & Outputs clean refusals (\textit{"There is no X"}) \\
\textbf{Counter-Attribute Traps} & 11.1\% (Prior Bias) & \textbf{16.7\%} & Demonstrates depth of MLP color priors \\
\textbf{Inception Hook ($\alpha = -0.35$)} & 0.0\% (Factual) & \textbf{100.0\% (Forced)} & Causally induces phantom hallucinations \\
\bottomrule
\end{tabular}
\caption{Comprehensive performance breakdown on the CounterPrior-50 Benchmark.}
\end{table}
